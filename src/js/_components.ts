export default (async () => {
  ...
  // sliderContent
  const sliderContentEls = document.querySelectorAll<HTMLElement>('.js-sliderContent');
  if (sliderContentEls.length > 0) {
    const { default: sliderContent } = await import(
      '../../templates/_components/slider/sliderContent'
    );
    sliderContent.init(sliderContentEls);
  }
